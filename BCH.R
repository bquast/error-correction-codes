...
for (i in 1:(2*t)) {
  # Create a minimal polynomial for alpha^i: (x + alpha^i)
...
codeword_with_error[3] <- (codeword_with_error[3] + 1) %% 2
...
if (all(syndromes == 0)) {
  ...
  # Simple error location and correction (manual, not using Berlekamp-Massey)
  error_position <- NULL
  for (i in 1:n) {
    syndrome_sum <- 0
    for (j in 1:(2*t)) {
  ...
  if (!is.null(error_position)) {
    codeword_with_error[error_position] <- (codeword_with_error[error_position] + 1) %% 2
  }
}
